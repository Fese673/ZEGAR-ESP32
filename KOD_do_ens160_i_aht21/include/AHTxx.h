...
/* Diagnostic application-level status.
    This library now derives it conservatively from documented status bits:
    - NO_VALID_OUTPUT: sensor busy or not calibrated yet
    - NORMAL_OPERATION: calibration bit set and sensor not busy
*/
...
/* Timing (used by state machine, NOT as blocking delay) */
#define AHTXX_CMD_DELAY_MS        10
#define AHTXX_MEASUREMENT_MS      85   // datasheet says 80ms typ, we add margin
...
class AHTxx {
  public:
    ...
    // Non-blocking init — starts state machine; Wire.begin() must have been called before
    bool begin(int32_t sda = SDA, int32_t scl = SCL,
               uint32_t speed = 100000, uint32_t stretch = 1000);

    // Drive the state machine — call every loop(). Returns true when NEW filtered data available
    bool update();

    // Is the sensor initialized and operating?
    bool isReady() const { return _state == AHT_STATE_READY; }

    // Filtered (median + Kalman + self-heat compensated) values
    float getTemperature() const { return _filteredTemp; }
    float getHumidity()    const { return _filteredHum; }

    // Raw values from last successful read
    float getRawTemperature() const { return _rawTemp; }
    float getRawHumidity()    const { return _rawHum; }

    // Legacy blocking API for drop-in compatibility
  ...
  private:
    ...
    uint8_t   _rawData[7];
    AHTState  _state;
    unsigned long _stateTs;        // timestamp for current state
    unsigned long _lastMeasMs;     // last measurement start time
    unsigned long _lastGoodSampleMs;
    unsigned long _intervalMs;     // min interval between measurements
    ...
    // Non-blocking I2C helpers (return immediately)
    bool _sendResetCmd();
