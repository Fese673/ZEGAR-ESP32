...
{
  return I2cShared::probe(_wire, _deviceAddress, _transactionTimeoutMs, _maxRetries);
}
...
int I2C_eeprom::_WriteBlock(const uint16_t memoryAddress, const uint8_t * buffer, const uint16_t length)
{
  ...
  uint8_t prefix[2] = {0};
  uint8_t prefixLen = 0;
  _buildAddressPrefix(memoryAddress, targetAddress, prefix, prefixLen);

  uint8_t tx[I2C_BUFFERSIZE + 2];
  ...
  for (uint16_t i = 0; i < length; i++)
  {
    tx[prefixLen + i] = buffer[i];
  }

  bool ok = I2cShared::write(_wire,
                             targetAddress,
                             tx,
                             prefixLen + length,
                             true,
...
uint8_t I2C_eeprom::_ReadBlock(const uint16_t memoryAddress, uint8_t * buffer, const uint16_t length)
{
  ...
  bool ok = I2cShared::writeRead(_wire,
                                 targetAddress,
                                 prefix,
                                 prefixLen,
                                 buffer,
...
bool I2C_eeprom::_verifyBlock(const uint16_t memoryAddress, const uint8_t * buffer, const uint16_t length)
{
  ...
  uint8_t tmp[I2C_BUFFERSIZE];
  uint8_t readBytes = _ReadBlock(memoryAddress, tmp, length);
  if (readBytes != length)
  {
    return false;
  }
  return memcmp(tmp, buffer, length) == 0;
}


bool I2C_eeprom::_isWriteWindowReady() const
{
...
void I2C_eeprom::_buildAddressPrefix(const uint16_t memoryAddress, uint8_t &address, uint8_t *prefix, uint8_t &prefixLen)
{
  if (this->_isAddressSizeTwoWords)
  {
    address = _deviceAddress;
    prefix[0] = (uint8_t)(memoryAddress >> 8);
    prefix[1] = (uint8_t)(memoryAddress & 0xFF);
    prefixLen = 2;
  }
  else
  {
    address = _deviceAddress | ((memoryAddress >> 8) & 0x07);
    prefix[0] = (uint8_t)(memoryAddress & 0xFF);
    prefixLen = 1;
  }
}
