...
bool AHTxx::begin(int32_t sda, int32_t scl, uint32_t speed, uint32_t stretch)
{
    ...
    // Start init sequence: power-on wait
...
// ─── Main state machine — call every loop() ─────────────────────────────────
bool AHTxx::update()
{
        ...
        if ((now - _stateTs) >= AHTXX_CMD_DELAY_MS) {
            ...
            if (_readStatusByte(st)) {
                if (st & AHTXX_STATUS_CTRL_CAL_ON) {
                    ...
                    _lastMeasMs = 0; // force first measurement
                    _measureCount = 0;
                } else {
                    _status = AHTXX_ERROR;
                    _invalidateSample(true);
    ...
    case AHT_STATE_READY:
        ...
        if (_lastMeasMs == 0 || (now - _lastMeasMs) >= _intervalMs) {
            if (_sendMeasurementCmd()) {
                ...
                _status = AHTXX_ACK_ERROR;
                _invalidateSample(true);
                // Stay in ready, try again next loop
            }
        }
        return false;

    case AHT_STATE_MEAS_CMD_SENT:
        // Wait for measurement to complete (85 ms typ)
    ...
    case AHT_STATE_MEAS_WAIT: {
        ...
        if (st & AHTXX_STATUS_CTRL_BUSY) {
            // Still busy — check timeout (max 200ms total)
            if ((now - _stateTs) > 200) {
        ...
        // 1) Self-heating compensation: subtract estimated offset
        //    The offset grows with measurement frequency; at 3s interval ~0.1°C
        float compensatedT = rawT - _selfHeatOffset;

        // 2) Median filter — rejects single-sample spikes/outliers
        float medT = _medTemp.update(compensatedT);
        float medH = _medHum.update(rawH);

        // 3) Kalman filter — optimal estimation with noise model
...
float AHTxx::readTemperature(bool readAHT)
{
    if (readAHT == AHTXX_FORCE_READ_DATA) {
        // Force a blocking measurement (emulate old behavior using state machine)
...
{
    if (readAHT == AHTXX_FORCE_READ_DATA) {
        // Same as readTemperature with force
