...
void SerialPM::init()
{
  ...
  // Note: skip flush on ESP32 to avoid blocking; buffers are usually small
  #ifndef ESP32
  uart->flush();
  #endif
  
  waitNonBlocking(50);  // non-blocking wait (50ms sufficient for passive mode setup)
  
  #ifndef ESP32
  // Clear RX buffer (non-blocking on ESP32 due to yield in wait)
  #endif
...
void SerialPM::sleep() {
  ...
  #ifndef ESP32
  uart->flush();  // only flush on non-ESP32 platforms to avoid blocking
  #endif
  waitNonBlocking(50);  // non-blocking wait for sleep command to take effect
}
...
void SerialPM::wake() {
  ...
  uart->flush();  // only flush on non-ESP32 platforms to avoid blocking
  #endif
  waitNonBlocking(50);  // non-blocking wait for wake command to take effect
}
...
SerialPM::STATUS SerialPM::trigRead()
{
  ...
  // Note: skip flush on ESP32 to avoid blocking; small buffer will transmit quickly
  #ifndef ESP32
  uart->flush();
  #endif
  ...
  // read message header - non-blocking read
  nbytes = 0;
  while (nbytes < headLen && uart->available())
  {
    buffer[nbytes] = uart->read();
    nbytes++;
    yield();  // allow other tasks to run between reads
  }
  if (nbytes != headLen)
  ...
  // read message body - non-blocking read
  uint8_t body_bytes = 0;
  while (body_bytes < bodyLen && uart->available())
  {
    buffer[headLen + body_bytes] = uart->read();
    body_bytes++;
    yield();  // allow other tasks to run between reads
  }
  nbytes += body_bytes;
...
void SerialPM::waitNonBlocking(uint16_t ms)
{
  // Non-blocking wait for ESP32: yield to FreeRTOS scheduler instead of blocking
  #ifdef ESP32
