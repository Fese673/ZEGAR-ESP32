...
bool getHourlyForecast(OM_HourlyForecast *structure, float latitude, float longitude, uint64_t unixTime, const char* apiLink = HOURLY_API_LINK);
...
bool getAirQualityForecast(OM_AirQualityForecast *structure, float latitude, float longitude, uint64_t unixTime, const char* apiLink = AIR_HOURLY_API_LINK);
