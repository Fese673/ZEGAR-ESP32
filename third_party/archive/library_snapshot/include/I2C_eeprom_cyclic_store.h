...
public:
    ...
    bool format()
    {
        if (!_isInitialized || _eeprom == nullptr)
            return false;
        ...
        const uint16_t slotsPerChunk = 8;
        for (uint16_t base = 0; base < totalSlots; base += slotsPerChunk)
        {
            uint16_t end = base + slotsPerChunk;
            if (end > totalSlots)
                end = totalSlots;

            for (uint16_t slot = base; slot < end; slot++)
            {
    ...
    {
        if (!_isInitialized || _eeprom == nullptr || buffer == nullptr)
            return false;
...
private:
    ...
    bool initialize()
    {
        if (_eeprom == nullptr || _pageSize == 0 || _totalPages == 0)
            return false;
